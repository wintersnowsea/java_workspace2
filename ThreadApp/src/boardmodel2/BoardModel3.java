...
import boardmodel.BoardMain;

//오라클의 Board 테이블에 관한 정보를 가진 모델 객체
public class BoardModel3 extends AbstractTableModel{
	BoardMain3 boardMain3;
	String[] column= {"board_id","title","writer","content","regdate","hit"};
	String[][] data;
	
	public BoardModel3(BoardMain3 boardMain3) {
		this.boardMain3=boardMain3;
		select();
	}
	public void select() {
		...
		try {
			//쿼리객체 생성
			pstmt=boardMain3.con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
	...
	public int insert(String title, String writer, String content) {
		...
		try {
			pstmt=boardMain3.con.prepareStatement(sql);
	...
	public int  delete(int board_id) {
		...
		try {
			...
			boardMain3.release(pstmt);//자원해제
		}
		return result;
	}
	
	public int update(String title2, String writer2, String content2, int board_id) {
		PreparedStatement pstmt=null;
		String sql="update board set title='"+title2+"', writer='"+writer2+"', content='"+content2+"'";
