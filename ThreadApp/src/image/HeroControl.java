...
//사용중인 OS의 디렉토리를 명시하는 것보다 클래스패스를 통한 차원접근이 더 자바스럽고,
//플랫폼에 중립적이다
//따라서 자원들도 package에 담을 수 있다
public class HeroControl extends JFrame {
	JPanel p;
	Image image;
	ImageManager imageManager;
	//패키지에는 꼭 클래스만 넣을 수 있는 게 아니다!
	//만일 패키지에 넣은 파일이 클래스인 경우
	//원래대로 패키지와 패키지 구분을 (.)점을 찍어서 구분하고
	//만일 패키지에 넣은 파일이 클래스 이외의 파일인 경우
	//클래스가 아니므로 디렉토리 취급한다
	//따라서 (.)점이 아닌 (/)로 구분해야한다
	String[] imgName= {
			"res/hero/image1.png",
	...
	public HeroControl() {
		...
		p=new JPanel() {
			protected void paintComponent(Graphics g) {
				...
				//this는 익명함수를 말하기 때문에 어떤 클래스의 this인지 명시가능
